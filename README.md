...
| Команда | Что делает | Пример |
|---------|-----------|--------|
| `theme` | Список доступных тем | `excli theme` |

Тема задаётся глобальным флагом `--theme` и влияет на все создаваемые элементы:

```bash
excli --theme sketch flow "A -> B -> C"
excli --theme clean box "Сервер" --at 100,200
excli --theme blueprint render arch.excli.yaml --replace
```

| Тема | Описание | Когда использовать |
|------|----------|--------------------|
| `default` | Helvetica, hand-drawn, solid fill | Универсальный вариант, подходит для всего |
| `sketch` | Virgil (рукописный), грубые линии, штриховка | Брейншторм, черновики, неформальные обсуждения |
| `clean` | Гладкие линии, скруглённые углы | Финальная документация, README, клиентские презентации |
| `bold` | Толстые линии, акцент | Презентации, слайды — видно издалека |
| `minimal` | Тонкие линии, минимализм | Встраивание в документы, когда диаграмма не главное |
| `blueprint` | Моноширинный шрифт, cross-hatch | Технические схемы, API-дизайн, системная архитектура |
| `whiteboard` | Virgil, hand-drawn | Обсуждение в команде, планирование, как на маркерной доске |
| `dots` | Точечная заливка, гладкие линии | Инфографика, отчёты с визуальным акцентом |
| `dashed` | Пунктирные линии, штриховка | Черновые/планируемые компоненты, "будет позже" |

#### Подсказки для выбора

**Для человека:** просто попробуй несколько тем на одной диаграмме и выбери, что нравится:
```bash
excli --theme clean flow "A -> B -> C" --clear
# Не понравилось? Попробуй другую:
excli --theme sketch flow "A -> B -> C" --clear
```

**Для AI-агента:** добавь в инструкции (CLAUDE.md или промпт) рекомендации:
```markdown
## Выбор темы excli
- Рабочие диаграммы для обсуждения → --theme whiteboard
- Документация и README → --theme clean или --theme minimal
- Техническая архитектура → --theme blueprint
- Презентации → --theme bold
- Черновики и брейншторм → --theme sketch
- Если не уверен → не указывай --theme (будет default)
```

Или проще — дай агенту одну строку:
```
Для excli используй --theme clean для финальных диаграмм и --theme sketch для черновиков.
```
...
|------|-------------|-----------|
| `--theme` | Глобальный | Визуальная тема: sketch, clean, bold и т.д. |
...
### Темы
- Черновик/обсуждение: excli --theme sketch flow "..."
- Финальная диаграмма: excli --theme clean flow "..."
- Техническая схема: excli --theme blueprint render arch.excli.yaml
- Презентация: excli --theme bold flow "..."
- Если не уверен — не указывай --theme (будет default)
